"use client";

import { useState, useRef, useEffect, use, useCallback } from "react";
import { Copy, Check } from "lucide-react";
import { Input } from "@/components/ui/input";
import { XTerm, type XTermHandle } from "@/components/terminal/XTerm";
import { serversApi } from "@/modules/servers/api";
import { io, Socket } from "socket.io-client";

interface ServerStats {
  running: boolean;
  daemonOffline: boolean;
  uptime?: number;
  startedAt?: string;
}

export default function ServerConsolePage({
  params,
}: {
  params: Promise<{ uuid: string }>;
}) {
  const { uuid } = use(params);
  const termRef = useRef<XTermHandle>(null);
  const socketRef = useRef<Socket | null>(null);
  const [input, setInput] = useState("");
  const [copied, setCopied] = useState(false);
  const [cmdHistory, setCmdHistory] = useState<string[]>([]);
  const [historyIdx, setHistoryIdx] = useState(-1);
  const [stats, setStats] = useState<ServerStats | null>(null);
  const [serverIp, setServerIp] = useState<string>("—");

  // Fetch server info for IP
  useEffect(() => {
    serversApi.get(uuid).then((s) => {
      try {
        const ports = JSON.parse(s.Ports ?? "[]") as { primary?: boolean; Port: string }[];
        const primary = ports.find((p) => p.primary)?.Port ?? ports[0]?.Port ?? "—";
        setServerIp(`${s.node.address}:${primary.split(":")[0]}`);
      } catch {
        setServerIp("—");
      }
    }).catch(() => {});
  }, [uuid]);

  // Poll server status
  useEffect(() => {
    let cancelled = false;
    async function poll() {
      try {
        const s = await serversApi.getStatus(uuid);
        if (!cancelled) setStats(s);
      } catch { /* ignore */ }
    }
    poll();
    const id = setInterval(poll, 5000);
    return () => { cancelled = true; clearInterval(id); };
  }, [uuid]);

  // Connect Socket.IO to console namespace
  useEffect(() => {
    const apiUrl = process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:4000";
    const socket = io(`${apiUrl}/console`, {
      query: { uuid },
      withCredentials: true,
      transports: ["websocket"],
    });
    socketRef.current = socket;

    socket.on("connect", () => {
      termRef.current?.writeln("\x1b[32m[Connected]\x1b[0m");
    });

    socket.on("output", (text: string) => {
      termRef.current?.write(text);
    });

    socket.on("disconnect", () => {
      termRef.current?.writeln("\x1b[33m[Disconnected]\x1b[0m");
    });

    socket.on("connect_error", () => {
      termRef.current?.writeln("\x1b[31m[Connection error]\x1b[0m");
    });

    return () => { socket.disconnect(); };
  }, [uuid]);

  const sendCommand = useCallback(() => {
    const cmd = input.trim();
    if (!cmd) return;
    termRef.current?.writeln(`\x1b[1m\x1b[33m~ \x1b[0m${cmd}`);
    socketRef.current?.emit("input", { command: cmd });
    setCmdHistory((prev) => [...prev.slice(-49), cmd]);
    setHistoryIdx(-1);
    setInput("");
  }, [input]);

  function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    if (e.key === "Enter") { sendCommand(); return; }
    if (e.key === "ArrowUp") {
      e.preventDefault();
      setCmdHistory((h) => {
        const idx = Math.min(historyIdx + 1, h.length - 1);
        setHistoryIdx(idx);
        setInput(h[h.length - 1 - idx] ?? "");
        return h;
      });
    }
    if (e.key === "ArrowDown") {
      e.preventDefault();
      const idx = Math.max(historyIdx - 1, -1);
      setHistoryIdx(idx);
      setInput(idx === -1 ? "" : cmdHistory[cmdHistory.length - 1 - idx] ?? "");
    }
  }

  function copyIP() {
    navigator.clipboard.writeText(serverIp);
    setCopied(true);
    setTimeout(() => setCopied(false), 2000);
  }

  const statusLabel = stats?.daemonOffline
    ? "Offline"
    : stats?.running
    ? "Running"
    : "Stopped";

  function formatUptime(seconds?: number) {
    if (!seconds) return "—";
    const h = Math.floor(seconds / 3600);
    const m = Math.floor((seconds % 3600) / 60);
    return h > 0 ? `${h}h ${m}m` : `${m}m`;
  }

  return (
    <div className="flex flex-col lg:flex-row gap-5">
      {/* Console */}
      <div className="w-full lg:w-2/3 flex flex-col min-w-0">
        <div className="flex flex-col rounded-xl overflow-hidden border border-neutral-800 shadow-lg">
          <div className="flex items-center gap-2 px-3 py-2 bg-[#1c1c1c] border-b border-neutral-800 shrink-0">
            <span className="text-[11px] font-medium text-neutral-600 select-none tracking-wide">console</span>
          </div>
          <XTerm ref={termRef} className="h-[420px] w-full" />
          <Input
            value={input}
            onChange={(e) => setInput(e.target.value)}
            onKeyDown={handleKeyDown}
            placeholder="Type a command..."
            className="rounded-none border-0 border-t border-neutral-800 bg-[#1a1a1a] text-neutral-200 placeholder:text-neutral-600 focus-visible:ring-0 focus-visible:border-neutral-700 font-mono text-sm"
            autoComplete="off"
            spellCheck={false}
          />
        </div>
      </div>

      {/* Stats */}
      <div className="w-full lg:w-1/3 space-y-4">
        {[
          {
            label: "IP Address",
            content: (
              <div className="flex items-start justify-between gap-2">
                <p className="text-sm font-medium font-mono text-neutral-800 dark:text-white break-all">{serverIp}</p>
                <button
                  onClick={copyIP}
                  className="shrink-0 rounded-lg p-1.5 bg-neutral-100 dark:bg-neutral-700/50 hover:bg-neutral-200 dark:hover:bg-neutral-600/50 transition-colors"
                >
                  {copied ? <Check className="h-3.5 w-3.5 text-emerald-500" /> : <Copy className="h-3.5 w-3.5 text-neutral-500" />}
                </button>
              </div>
            ),
          },
          { label: "Status", content: <p className="text-lg font-medium text-neutral-800 dark:text-white">{statusLabel}</p> },
          { label: "Uptime", content: <p className="text-lg font-medium text-neutral-800 dark:text-white">{formatUptime(stats?.uptime)}</p> },
        ].map((card) => (
          <div
            key={card.label}
            className="bg-white dark:bg-neutral-800/50 border border-neutral-200 dark:border-neutral-700/30 rounded-xl px-4 py-4 shadow-sm"
          >
            <h2 className="text-sm font-medium text-neutral-500 dark:text-neutral-400 mb-1">{card.label}:</h2>
            {card.content}
          </div>
        ))}
      </div>
    </div>
  );
}
