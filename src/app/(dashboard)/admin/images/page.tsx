...
const mockImages = [
  { id: 1, name: "Minecraft Java", author: "AirlinkLabs", createdAt: "2025-01-01" },
  { id: 2, name: "Velocity Proxy", author: "AirlinkLabs", createdAt: "2025-01-05" },
  { id: 3, name: "Paper MC", author: "community", createdAt: "2025-02-10" },
];

export default function AdminImagesPage() {
  const [filter, setFilter] = useState("");
  const filtered = mockImages.filter((i) => i.name.toLowerCase().includes(filter.toLowerCase()));
    ...
    <>
      ...
      <div className="flex border-b border-neutral-200 dark:border-neutral-800 mb-5">
        <span className="px-1 py-2.5 mr-5 text-sm font-medium text-neutral-800 dark:text-white border-b-2 border-neutral-800 dark:border-white -mb-px">
          Installed <span className="ml-1 text-xs text-neutral-400">{mockImages.length}</span>
        </span>
        <Link href="/admin/images/store" className="px-1 py-2.5 text-sm font-medium text-neutral-400 hover:text-neutral-600 dark:hover:text-neutral-300 border-b-2 border-transparent -mb-px transition">
          Store
        </Link>
      </div>
